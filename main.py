...
async def join_request(message: types.Message):
    ...
    message_data = await bot.send_message(message.from_user.id, setting.capcha_text, reply_markup=get_apply_markup())
...
@dp.message_handler(text=setting.capcha_btn_text)
...
async def resend_capcha(message_data, user_id):
    ...
    if user_id in users_verifying:
        message_data = await bot.send_message(message_data.chat.id, message_data.text, reply_markup=get_apply_markup())
        await resend_capcha(message_data, user_id)
...
@dp.callback_query_handler(text="settings_capcha")
async def _verification_set(callback: CallbackQuery, state: FSMContext):
    await bot.send_message(callback.from_user.id,"Настройка капчи", reply_markup=get_capcha_markup())
...
@dp.callback_query_handler(text="capcha_btn_text")
async def _verification_set(callback: CallbackQuery, state: FSMContext):
    await VerificationSet.capcha_btn.set()
    await bot.send_message(callback.from_user.id, "Введите что будет отображаться на конопке капчи")

@dp.message_handler(state=VerificationSet.capcha_btn)
async def _verification_set(message: types.Message, state: FSMContext):
    setting: Setting = Setting.get(id=1)
    setting.capcha_btn_text = message.text
    setting.save()
...
def get_apply_markup():
    setting: Setting = Setting.get(id=1)
    markup = ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(KeyboardButton(setting.capcha_btn_text))
    return  markup
...
def get_capcha_markup():
    ...
    markup.add(InlineKeyboardButton("Изменить текст кнопки", callback_data="capcha_btn_text"))
    return markup
